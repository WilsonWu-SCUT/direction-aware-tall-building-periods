"""Regression primitives used to reproduce manuscript Section 4.2."""
...
def load_directional_observations(csv_path: Path) -> list[Observation]:
    """Expand each building row into paired transverse and longitudinal records."""
            ...
            for direction in ("transverse", "longitudinal"):
                observation = Observation(
                    **common,
                    direction=direction,
                    effective_width_m=float(
                        row[f"{direction}_effective_width_m"]
                    ),
