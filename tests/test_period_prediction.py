"""Regression tests for the period-prediction workflow."""
...
from analysis.period_prediction.regression import (
    CATEGORY_ORDER,
    CONSTRAINTS,
    fit_constrained,
    fit_unconstrained,
...
DATA = ROOT / "data" / "period_prediction" / "tall_building_periods.sqlite"
CSV_DATA = ROOT / "data" / "period_prediction" / "tall_building_periods.csv"


class PeriodPredictionRegressionTests(unittest.TestCase):
    @classmethod
    ...
    def test_sqlite_and_csv_inputs_are_equivalent(self) -> None:
        csv_observations = load_directional_observations(CSV_DATA)
        key = lambda item: (item.model_id, item.direction)
        self.assertEqual(
            sorted(self.observations, key=key),
            sorted(csv_observations, key=key),
        )
